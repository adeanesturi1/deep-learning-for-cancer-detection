...
# Sort by the 'Dice' column (case-sensitive)
df_sorted = df.sort_values(by='Dice', ascending=True) # <-- UPDATED
...
print(patients_to_plot[['Patient', 'Dice']]) # <-- UPDATED
...
for index, row in patients_to_plot.iterrows():
    ...
    patient_id = str(row['Patient']).strip() # <-- UPDATED
    dice_score = row['Dice']               # <-- UPDATED
