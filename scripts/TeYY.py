...
try:
    ...
    print("NOTE: torchviz also requires the Graphviz system library (e.g., 'sudo apt-get install graphviz')")
    make_dot = None # Allows the script to run without this library
...
# --- NEW: Set this to False if you do not have a surrogate model (e.g., fold_0) ---
...
try:
    ...
    # --- UPDATED: Conditionally load the surrogate model ---
...
if __name__ == "__main__":
    ...
    # --- Phase 1: Optional Model Visualization ---
    if PERFORM_VISUALIZATION:
        print("\n--- Starting Optional Model Visualization ---", flush=True)
        if summary is None and make_dot is None:
            print("Skipping visualization: torchsummary and torchviz are not installed.", flush=True)
        else:
            model_to_visualize = models['black_box_fold3']
            print(f"Visualizing model: 'black_box_fold3'")

            # Determine input size for visualization from plans file
            num_input_channels = len(dataset_json_content['channel_names'])
            patch_size = plans_json_content['configurations']['3d_fullres']['patch_size']
            input_size_for_summary = (num_input_channels, *patch_size)
            print(f"Using representative input size (C, D, H, W): {input_size_for_summary}")

            # Method 1: torchsummary (Recommended)
            if summary:
                print("\n" + "="*80)
                print(" METHOD 1: Model Summary using torchsummary (Text-based)")
                print("="*80)
                summary(model_to_visualize, input_size=input_size_for_summary, batch_size=1, device=str(device), depth=4)
            
            # Method 2: torchviz (Graph)
            if make_dot:
                print("\n" + "="*80)
                print(" METHOD 2: Computation Graph using torchviz")
                print("="*80)
                try:
                    dummy_input = torch.randn(1, *input_size_for_summary).to(device)
                    output = model_to_visualize(dummy_input)
                    graph = make_dot(_get_logits_from_nnunet_output(output), params=dict(model_to_visualize.named_parameters()))
                    output_filename = "bcbm_model_graph"
                    graph.render(output_filename, format="png", cleanup=True)
                    print(f"Success! Model graph saved to '{output_filename}.png'")
                except Exception as e:
                    print(f"Failed to generate torchviz graph. Error: {e}")

            # Method 3: Basic print(model)
            print("\n" + "="*80)
            print(" METHOD 3: Basic Module Hierarchy using print(model)")
            print("="*80)
            print(model_to_visualize)
        print("\n--- Visualization complete ---", flush=True)

    # --- Phase 2: (Optional) Adversarial Fine-Tuning ---
    patient_cases_for_attack = [f"bcbm_{i:03d}" for i in range(30, 60)]
    slices_to_attack = [75]
    
    if PERFORM_ADVERSARIAL_TRAINING:
        tuning_patient_cases = [f"bcbm_{i:03d}" for i in range(1, 11)]
    ...
    # --- Phase 3: Define All Attack Configurations ---
    ...
    for tm_name in target_models:
        ...
        # --- UPDATED: Conditionally add black-box attacks ---
    ...
    # --- Phase 5: Save and Analyze Results ---
    ...
    # --- Phase 6: Visualization & Statistical Analysis ---
