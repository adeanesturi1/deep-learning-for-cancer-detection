...
def remap_first_conv(w, in_ch):
    # average pretrained 4-channel conv weights down to in_ch
    m = w.mean(dim=1, keepdim=True)
    if in_ch==1:
        return m
    return m.repeat(1,in_ch,1,1,1)
