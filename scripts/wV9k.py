...
def _get_logits_from_nnunet_output(output):
    ...
    if isinstance(output, (list, tuple)):
        # If it's a list or tuple, assume the first element is the main output.
        # Recursively call in case it's a nested list/tuple.
        return _get_logits_from_nnunet_output(output[0])
    elif isinstance(output, torch.Tensor):
        # If it's a tensor, we found our logits.
        return output
    else:
        # Unexpected type
...
# Use the helper function for adversarial attack
