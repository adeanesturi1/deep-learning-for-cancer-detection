...
# ---------------------------------------------------------------------
#  Dataset: central axial slice + augment + normalize + stronger aug
# ---------------------------------------------------------------------
class BCBM2DDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # augmentations
        if self.augment and TF is not None:
            # random rotation
            ...
            seg_t = TF.resize(seg_t, [new_h, new_w], interpolation=InterpolationMode.NEAREST)
        ...
        elif self.augment and TF is None:
            # simple horizontal flip fallback
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze_epochs", type=int, default=5,
                        help="Initial epochs to keep encoder frozen")
    ...
    for name, param in model.named_parameters():
        if name.startswith("enc1") or name.startswith("enc2") or \
           name.startswith("enc3") or name.startswith("enc4") or \
           name.startswith("bot"):
    ...
    # initial optimizer on decoder + heads only
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if not unfroze and epoch == args.freeze_epochs + 1:
            ...
            remaining_steps = (args.epochs - epoch + 1) * len(train_dl)
        ...
        # visualize probability heatmap every 5 epochs
        if epoch == 1 or epoch % 5 == 0:
            img_np = img[0,0].cpu().numpy()
            prob_map = p[0,0].cpu().numpy()
            fig, ax = plt.subplots(1,2, figsize=(8,4))
            ax[0].imshow(img_np, cmap='gray'); ax[0].set_title('Input')
            ax[1].imshow(img_np, cmap='gray')
            ax[1].imshow(prob_map, cmap='jet', alpha=0.5); ax[1].set_title('Prob heatmap')
            fig.tight_layout()
            fig.savefig(f'heatmap_epoch{epoch:03d}.png')
            plt.close(fig)
    ...
    plt.plot(epochs_arr, history['val_soft_dice'], label='Val Soft Dice')
    plt.plot(epochs_arr, history['val_auc'],       label='Val AUC')
