    ...
    for epoch in range(1, args.epochs+1):
        ...
        with torch.no_grad():
            for imgs, segs, labs in val_dl:
                ...
                s_out, c_out = model(imgs)
                # segmentation metric on soft outputs
                p_seg = torch.sigmoid(s_out)
                batch_dice = ((2*(p_seg*segs).sum([2,3]) + 1e-6) /
                              ((p_seg + segs).sum([2,3]) + 1e-6)).mean().item()
        ...
        # compute AUC using classifier outputs
