...
# ---------------------------------------------------------------------
#  Main training loop with dynamic workers & debug prints
# ---------------------------------------------------------------------
if __name__ == "__main__":
    ...
    parser.add_argument("--num_workers",  type=int, default=8,
                        help="workers for training DataLoader")
    parser.add_argument("--val_workers",  type=int, default=4,
                        help="workers for validation DataLoader")
    ...
    print(f">>> Dataloader workers: train={args.num_workers}, val={args.val_workers}, augment={args.augment}", flush=True)
    ...
    # Data loaders with caching, dynamic workers, pin_memory
    ...
    val_dl   = DataLoader(
        ...
        shuffle=False, num_workers=args.val_workers, pin_memory=True,
        prefetch_factor=2, collate_fn=collate_fn
    )

    # debug: sample one batch
    print(">>> Sampling one batch for timing...", flush=True)
    try:
        t0 = time.time()
        _ = next(iter(train_dl))
        print(f">>> Batch sample time: {time.time() - t0:.2f} sec", flush=True)
    except Exception as e:
        print(f"!!! Batch sampling failed: {e}", flush=True)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for imgs, segs, labs in train_dl:
            ...
            print(f"Batch load + to-device: {time.time() - t_start:.2f}s", flush=True)
        ...
        if mask.sum() < len(probs_arr): warnings.warn(f"Dropped {len(probs_arr)-mask.sum()} NaNs before AUC.")
