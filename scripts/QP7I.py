...
class BCBM3DDataset(Dataset):
    def __init__(self, raw_dir, preproc_dir, metadata_csv, target_z):
        ...
        self.Z         = target_z
        self.num_mods  = 4  # number of modalities
    ...
    def __getitem__(self, i):
        ...
        for m in range(self.num_mods):
            fn = f"{case_id}_{m:04d}.nii.gz"
            ip = os.path.join(self.img_dir, fn)
            img = nib.load(ip).get_fdata().astype(np.float32)  # (H,W,D)
            img = self._pad_or_crop(img)
            mods.append(img)
        img = np.stack(mods, axis=0)       # (4,H,W,Z)
        img = torch.from_numpy(img)        # float32

        # load preprocessed GT
        sp = os.path.join(self.seg_dir, f"{case_id}.nii.gz")
        seg = nib.load(sp).get_fdata().astype(np.float32)
...
def train_epoch(model, loader, opt, device):
    ...
    for img, seg_gt, her2 in loader:
        ...
        loss = lseg(seg_logits, seg_gt) + 0.5*lcls(cls_logits, her2)
        loss.backward()
        opt.step()
        total += loss.item()
