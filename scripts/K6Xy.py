...
class BCBM2DDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.augment and TF:
            ...
            seg_t = TF.resize(seg_t,(nh,nw),InterpolationMode.NEAREST)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if not unfroze and epoch == args.freeze_epochs + 1:
            ...
            optimizer = optim.AdamW(
                [{'params': dec, 'lr': args.lr}, {'params': enc, 'lr': args.lr * 0.1}],
                weight_decay=args.weight_decay
            )
            rem_steps = (args.epochs - epoch + 1) * len(train_dl)
            scheduler = OneCycleLR(optimizer, max_lr=args.lr, total_steps=rem_steps,
                                   pct_start=0.1, div_factor=10, final_div_factor=1e4)
            unfroze = True
        ...
        with torch.no_grad():
            for imgs, segs, labs in val_dl:
                ...
                batch_dice = ((2 * (p * segs).sum([2,3]) + 1e-6) /
                              ((p + segs).sum([2,3]) + 1e-6)).mean().item()
        ...
        if mask.sum() < len(probs_arr):
            warnings.warn(f"Dropped {len(probs_arr)-mask.sum()} NaN scores before AUC.")
        val_auc = roc_auc_score(labs_arr[mask], probs_arr[mask])
        history['dice'].append(val_dice)
        history['auc'].append(val_auc)

        # Log metrics
        print(f"Epoch {epoch:03d} Loss {train_loss:.4f} Dice {val_dice:.4f} AUC {val_auc:.4f}")
        ...
        if epoch == 1 or epoch % 5 == 0:
            ...
            ax[0].imshow(im_np, cmap='gray');  ax[0].set_title('Input')
            ax[1].imshow(im_np, cmap='gray');  ax[1].imshow(pm_np, cmap='jet', alpha=0.5)
            ax[1].set_title('Prob heatmap')
            fig.tight_layout()
