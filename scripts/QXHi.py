...
# === Original prediction ===
# When deep supervision is enabled, model returns a tuple of tensors (or sometimes list of tensors).
# The first element is typically the highest resolution output.
output_from_model = model(img_tensor_normalized)
# Check if the output is a tuple/list and extract the relevant tensor for argmax
if isinstance(output_from_model, (list, tuple)):
    # Assuming the first element is the main output tensor
    output_logits = output_from_model[0]
else:
    # If it's not a tuple/list, assume it's directly the logits tensor
    output_logits = output_from_model

pred_full_volume = output_logits.argmax(dim=1).squeeze(0) # Apply argmax to the extracted tensor
pred_slice = pred_full_volume[slice_idx, :, :]


# === FGSM Attack ===
img_tensor_attack = img_tensor_normalized.clone().detach().requires_grad_(True)
# Apply the same logic for the adversarial attack output
output_attack_from_model = model(img_tensor_attack)
if isinstance(output_attack_from_model, (list, tuple)):
    output_attack_logits = output_attack_from_model[0]
else:
    output_attack_logits = output_attack_from_model

loss = cross_entropy(output_attack_logits, gt_full_volume_tensor_for_loss) # Use extracted logits for loss
...
with torch.no_grad():
    adv_output_from_model = model(adv_image)
    if isinstance(adv_output_from_model, (list, tuple)):
        adv_output_logits = adv_output_from_model[0]
    else:
        adv_output_logits = adv_output_from_model
