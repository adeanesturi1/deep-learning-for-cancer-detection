...
class UNet3D(nn.Module):
    ...
    def forward(self, x):
        e1 = self.enc1(x)
        e2 = self.enc2(torch.nn.functional.max_pool3d(e1,2))
        e3 = self.enc3(torch.nn.functional.max_pool3d(e2,2))
        e4 = self.enc4(torch.nn.functional.max_pool3d(e3,2))
        b  = self.bot(torch.nn.functional.max_pool3d(e4,2))
...
class BCBM3DDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        img_p = os.path.join(self.raw, "imagesTr", f"{case_id}_0000.nii.gz")
        if not os.path.isfile(img_p):
            raise FileNotFoundError(f"Missing image: {img_p}")

        # Try both possible GT folders
        p1 = os.path.join(self.gt, "gt_segmentations", f"{case_id}.nii.gz")
        p2 = os.path.join(self.gt, "labelsTr",          f"{case_id}.nii.gz")
        if   os.path.isfile(p1): seg_p = p1
        elif os.path.isfile(p2): seg_p = p2
        else: raise FileNotFoundError(f"Missing seg for {case_id}")
        ...
        if Z0 > Zt:
            off = (Z0 - Zt)//2
            img, seg = img[:,:,off:off+Zt], seg[:,:,off:off+Zt]
        elif Z0 < Zt:
            pad = Zt - Z0; f = pad//2; b = pad - f
...
# Corrected remapping of BraTS -> UNet3D
...
def train_epoch(model, loader, opt, device, scaler):
    ...
    for img, seg_gt, her2 in loader:
        ...
        with torch.cuda.amp.autocast(enabled=(device.type=="cuda")):
            ...
            loss = sl(seg_logits,seg_gt) + 0.5*cl(cls_logits,her2)
...
def main():
    ...
    raw_sd = ckpt.get("network_weights", ckpt) if isinstance(ckpt,dict) else ckpt
    ...
    for ep in range(1, args.epochs+1):
        try:
            ...
            if "out of memory" in str(e).lower():
                print(f"OOM at epoch {ep} — lower base_feats/target_z or batch_size")
