...
for pid, fold in patients:
    ...
    t1ce_path = os.path.join(t1ce_path_base, f"{pid}_0001.nii.gz")  # T1ce

    if not all(os.path.exists(p) for p in [pred_path, gt_path]):
        print(f"Skipping {pid} — missing prediction or GT")
        continue

    if not os.path.exists(t1ce_path):
        print(f"T1ce not found for {pid}, skipping...")
        continue
...
print(f"All visualisations saved to: {outdir}")
