...
class BCBM3DDataset(Dataset):
    ...
    def __getitem__(self,i):
        ...
        img   = nib.load(os.path.join(self.raw, "imagesTr", f"{cid}_0000.nii.gz")).get_fdata().astype(np.float32)
        seg   = nib.load(os.path.join(self.gt,  f"{cid}.nii.gz")).get_fdata().astype(np.float32)
        H,W,Z = img.shape; tz = self.tz
        if Z>tz:
            s = (Z-tz)//2
            img, seg = img[:,:,s:s+tz], seg[:,:,s:s+tz]
        elif Z<tz:
...
if __name__=="__main__":
    ...
    p.add_argument("--patience",       type=int, default=10,
                   help="epochs w/o val improvement before stopping")
    args = p.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model  = UNet3D(1, args.base_feats).to(device)

    # --- load BraTS encoder into model (same as before) ---
    ...
    for epoch in range(1, args.epochs+1):
        ...
        model.train()
        running_loss = 0.0
        for imgs, segs, hers in dl_tr:
            imgs, segs, hers = imgs.to(device), segs.to(device), hers.to(device)
            opt.zero_grad()
            s_out, c_out = model(imgs)
            loss = seg_fn(s_out, segs) + 0.5*cls_fn(c_out, hers)
            loss.backward(); opt.step()
            running_loss += loss.item()
        train_loss = running_loss / len(dl_tr)
        history["train_loss"].append(train_loss)

        # ——— VALIDATION ———
        model.eval()
        dice_sum, acc_sum = 0.0, 0.0
        with torch.no_grad():
            for imgs, segs, hers in dl_val:
                imgs, segs, hers = imgs.to(device), segs.to(device), hers.to(device)
                s_out, c_out = model(imgs)
                # dice
                preds = (torch.sigmoid(s_out) > 0.5).float()
                inter = (preds * segs).sum(dim=(1,2,3))
                union = preds.sum(dim=(1,2,3)) + segs.sum(dim=(1,2,3))
                dice  = ((2*inter + 1e-6) / (union + 1e-6)).mean().item()
                dice_sum += dice
                # accuracy
                acc = ((torch.sigmoid(c_out)>0.5).float() == hers).float().mean().item()
                acc_sum += acc

        val_dice = dice_sum / len(dl_val)
        val_acc  = acc_sum  / len(dl_val)
        ...
        if val_dice > best_dice:
            ...
            print(f" → New best! (Dice {val_dice:.4f}) saved.", flush=True)
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= args.patience:
                print(f"No improvement for {args.patience} epochs, stopping early.", flush=True)
                break
    ...
    print("✓ Training complete. Metrics plot saved → metrics.png")
