...
class PlainConvUNet(AbstractDynamicNetworkArchitectures):
    def __init__(
        ...
        deep_supervision: bool = False,       # <--- we capture this
        nonlin_first: bool = False,
    ):
        super().__init__()
        # make the flag available in forward()
    ...
    def forward(self, x):
        ...
        # now safe to branch on deep_supervision
        if self.deep_supervision:
            return seg_output, bottleneck
        else:
            return seg_output, bottleneck
...
class ResidualEncoderUNet(AbstractDynamicNetworkArchitectures):
    def __init__(
        ...
        nonlin: Union[None, Type[torch.nn.Module]] = None,
        nonlin_kwargs: dict = None,
        deep_supervision: bool = False,       # <--- also here
        block: Union[Type[nn.Module], Type[nn.Module]] = BasicBlockD,
