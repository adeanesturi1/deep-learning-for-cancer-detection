...
# This file is usually in the nnUNet_raw data directory for the dataset.
# Please verify this path exists on your system for Dataset001_BraTS.
dataset_json_path = "/sharedscratch/an252/cancerdetectiondataset/nnUNet_raw/Dataset001_BraTS/dataset.json" # <--- VERIFY THIS PATH

# --- Device Setup ---
device = torch.device('cpu')

# === Load nnUNetTrainer
# Load dataset_json content
try:
...
try:
    ...
    print(f"Error loading image or GT file: {e}. Please check your paths.")
    exit()
...
def dice(gt_seg, pred_seg, label):
    ...
    if union_sum == 0:
        # If both ground truth and prediction are empty for this label, Dice is 1.0 (perfect agreement on absence)
        return 1.0
