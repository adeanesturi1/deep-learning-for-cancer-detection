...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs+1):
        print(f"\n>>> Starting Epoch {epoch:03d}", flush=True)
        ...
        print(f"   TrainLoss: {train_loss:.4f}", flush=True)
        print(f"   ValDice:   {val_dice:.4f}", flush=True)
        print(f"   ValAUC:    {val_auc:.4f}", flush=True)
        print(f"[Epoch {epoch:03d}] Best AUC so far: {best_score:.4f}", flush=True)
