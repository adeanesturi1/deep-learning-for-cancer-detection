...
# --- 1. SETTINGS (Based on the paths you provided) ---
...
for index, row in df.iterrows():
    ...
    # Construct all three required paths
...
if not valid_patients:
    print("ERROR: Could not find any patients with a complete set of GT, Prediction, and preprocessed MRI files.")
    exit()
...
print(patients_to_plot[['Patient', 'Dice']])

# --- 3. PREPARE PLOT STYLES ---
colors = [[0, 0, 0, 0], [1, 0, 0, 1]] # 0=background, 1=Tumor (red)
...
for index, row in patients_to_plot.iterrows():
    ...
    try:
        ...
        mri_data = np.load(mri_path)['data'][0] # Load .npz and get first channel
