...
# --- NEW: Helper function to extract a 3D patch ---
def get_random_patch(image_tensor, gt_tensor, patch_size):
    """
    Extracts a random 3D patch from the image and ground truth tensors.
    Tries to center the patch on a foreground class if possible.
    """
    ...
    # Try to find a foreground center
    foreground_coords = torch.argwhere(gt_tensor > 0)
    if len(foreground_coords) > 0:
        center_coord = random.choice(foreground_coords)
        # B, C, D, H, W -> we need D, H, W which are indices 2, 3, 4
        center_d, center_h, center_w = center_coord[2], center_coord[3], center_coord[4]
    else:
        # If no foreground, pick a random center
        center_d, center_h, center_w = d // 2, h // 2, w // 2

    # Calculate patch boundaries, ensuring they are within the image dimensions
...
# --- MODIFIED: Adversarial Fine-Tuning now uses patches to avoid OOM errors ---
    ...
    for epoch in range(2):
        ...
        for patient_id in tqdm(patient_ids_for_tuning, desc=f"Epoch {epoch+1}"):
            try:
                ...
                # --- Process one random patch per patient to save memory ---
                img_patch, gt_patch = get_random_patch(img_tensor_full, gt_tensor_full, patch_size)

                # --- Generate Adversarial Example using PGD on the patch ---
                ...
                # --- Training Step on the Adversarial Patch ---
                optimizer.zero_grad()
...
# --- MODIFIED: Attack Evaluation now uses patches to avoid OOM errors ---
def run_attack_evaluation(patient_id, slice_idx, attack_config, models_dict, device, output_base_dir,
    ...
    try:
        ...
        img_tensor_full = preprocess_single_image_for_nnunet(
            loaded_image_data, loaded_image_properties, plans_json_content, device
        )

        target_D, target_H, target_W = img_tensor_full.shape[2:]
        gt_tensor_full = torch.tensor(gt_full_volume_raw.astype(np.int64)).to(device).unsqueeze(0).unsqueeze(0)
        gt_preprocessed_full = torch.nn.functional.interpolate(gt_tensor_full.float(),
                                                          size=(target_D, target_H, target_W),
                                                          mode='nearest-exact').squeeze(0).long()
        
        # --- Extract a 3D patch centered around the slice_idx for evaluation ---
        patch_size = plans_json_content['configurations']['3d_fullres']['patch_size']
        pd, ph, pw = patch_size
        
        # Center the patch on the desired slice
        d_start = max(0, slice_idx - pd // 2)
        h_start = max(0, target_H // 2 - ph // 2)
        w_start = max(0, target_W // 2 - pw // 2)
        
        d_end = min(target_D, d_start + pd)
        h_end = min(target_H, h_start + ph)
        w_end = min(target_W, w_start + pw)
        ...
        # --- Generate Adversarial Example (on patch) ---
        adv_image_patch = None
        data_grad = None
        
        if attack_config['type'] == 'fgsm':
            img_patch.requires_grad = True
            output_logits_grad = _get_logits_from_nnunet_output(gradient_model(img_patch))
            loss = cross_entropy(output_logits_grad, gt_patch)
            gradient_model.zero_grad()
            loss.backward()
            data_grad = img_patch.grad.data
            adv_image_patch = img_patch.detach() + epsilon * data_grad.sign()
            ...
            for i in range(attack_config['num_iter']):
                ...
                adv_image_patch = img_patch.detach() + total_perturbation
        ...
        # --- Adversarial Prediction (on patch) ---
        with torch.no_grad():
        ...
        # --- Save Visualizations of the patch's central slice ---
        fig, axs = plt.subplots(1, 4, figsize=(20, 6))
        
        original_slice_np = img_patch[0, 0, patch_slice_idx, :, :].cpu().numpy()
        ...
        perturbation_slice_np = (adv_image_patch - img_patch)[0, 0, patch_slice_idx, :, :].cpu().numpy()
        ...
        adv_slice_np = adv_image_patch[0, 0, patch_slice_idx, :, :].cpu().numpy()
