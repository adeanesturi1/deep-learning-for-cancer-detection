...
# Find the top 3 patients that have a non-empty ground truth mask
top_3_with_tumors = []
...
for index, row in sorted_df.iterrows():
    ...
    try:
        ...
        # Check if the mask contains any segmentation (is not all zeros)
        if np.sum(gt_mask) > 0:
...
# Get the 3 worst (lowest Dice) patients
...
# Generate the legend separately as before
