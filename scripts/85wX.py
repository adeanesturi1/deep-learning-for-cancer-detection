...
if __name__ == "__main__":
    ...
    # Data loaders with parallelism & pinning
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if not unfroze and epoch == args.freeze_epochs+1:
            ...
            rem = (args.epochs - epoch + 1) * len(train_dl)
            scheduler = OneCycleLR(
                optimizer, max_lr=args.lr, total_steps=rem,
                pct_start=0.1, div_factor=10, final_div_factor=1e4
            )
            unfroze = True
        ...
        if mask.sum() < len(probs_arr):
            warnings.warn(f"Dropped {len(probs_arr)-mask.sum()} NaN before AUC.")
        ...
        # Log heatmap at first and last epoch
        ...
        if val_dice > best_dice:
            ...
            writer.add_text('Model', f'New best at epoch {epoch}', epoch)
        else:
            no_improve += 1
