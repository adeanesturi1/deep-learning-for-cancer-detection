...
for i, (x, y) in enumerate(positions):
    ...
    # Dice Drop bar
    ax.bar(x + 0.07, scaled_drop[i], width=0.05, color="#e41a1c")
    ax.text(x + 0.07, scaled_drop[i] + 0.02, f"{dice_drop_scores[i]:.2f}", ha='center', fontsize=8)
    ax.text(x + 0.07, -0.05, "Dice Drop", ha='center', fontsize=9)
...
legend_patches = [
    ...
    mpatches.Patch(color='#e41a1c', label='Dice Drop (higher = worse)')
]
...
# BraTS → BCBM
ax.annotate("",
    xy=(0.2, 1), xytext=(0.8, 1),
    arrowprops=arrow_props)

# BCBM → Adversarial BraTS (or directly from BraTS → Adversarial)
ax.annotate("",
    xy=(1.2, 1), xytext=(1.8, 1),
    arrowprops=arrow_props)
