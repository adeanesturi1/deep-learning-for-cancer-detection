...
def preprocess_single_image_for_nnunet(raw_image_data_np, raw_image_properties, plans_dict, device):
    ...
    # Corrected access based on your `loaded_image_properties.keys()` output:
    original_spacing_xyz = np.array(raw_image_properties['spacing']) # This is (X, Y, Z) order from SimpleITK
    
    # Access origin and direction from 'sitk_stuff'
    sitk_stuff = raw_image_properties['sitk_stuff']
    original_origin_xyz = sitk_stuff['origin']
    original_direction_matrix = sitk_stuff['direction'] # This is a tuple representing the 3x3 matrix
...
# The shape is (B, C, D, H, W), so we need D, H, W from indices 2, 3, 4
