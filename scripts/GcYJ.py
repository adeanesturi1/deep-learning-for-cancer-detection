...
# --- Configuration ---
# Set to True to run a brief adversarial fine-tuning session on the black-box model
PERFORM_ADVERSARIAL_TRAINING = True
# If True, a new model 'checkpoint_final_robust.pth' will be created and evaluated.
# If False, the script will skip fine-tuning and only run attacks on the original models.

# --- Global Settings & Paths for BCBM Dataset ---
# NOTE: These paths are configured for the BCBM dataset (Dataset002)
...
output_base_dir = "/sharedscratch/an252/cancerdetectiondataset/bcbm_attacks_multi_run_v1" # New output directory for BCBM results

# Checkpoint paths for BCBM model
# This is the target model we want to attack
...
# This is a different model used to generate gradients for black-box attacks.
# IMPORTANT: Assumes a model for fold_0 exists. Verify this path is correct.
...
# Path for the new adversarially trained model
...
try:
    ...
    trainer_black_box = nnUNetTrainer(plans=plans_path, configuration="3d_fullres", fold=3, dataset_json=dataset_json_content, unpack_dataset=False, device=device)
    trainer_black_box.initialize()
    checkpoint_black_box = torch.load(checkpoint_path_black_box, map_location=device)
    ...
    # NOTE: This model is used to craft transferable (black-box) attacks.
    trainer_surrogate = nnUNetTrainer(plans=plans_path, configuration="3d_fullres", fold=0, dataset_json=dataset_json_content, unpack_dataset=False, device=device)
    trainer_surrogate.initialize()
    checkpoint_surrogate = torch.load(checkpoint_path_surrogate, map_location=device)
    ...
    print("Ensure the checkpoint paths are correct and a surrogate model for fold_0 exists.", flush=True)
    exit()
...
# BCBM dataset typically has background (0) and metastasis (1)
TUMOR_REGIONS = {"Metastasis": [1]}

# --- Adversarial Fine-Tuning Function ---
def adversarial_fine_tuning(model_to_train, patient_ids_for_tuning, plans, attack_params, device, save_path):
    ...
    for epoch in range(2): # Fine-tune for 2 epochs
        ...
        for patient_id in tqdm(patient_ids_for_tuning, desc=f"Epoch {epoch+1}"):
            try:
                # Load and preprocess data (assuming single modality for BCBM)
                ...
                for _ in range(attack_params['num_iter']):
                    ...
                    adv_img_tensor = torch.clamp(adv_img_tensor, -5, 5) # Clamp to valid normalized range
    ...
    model_to_train.eval() # Set back to evaluation mode
...
def run_attack_evaluation(patient_id, slice_idx, attack_config, models_dict, device, output_base_dir,
    ...
    try:
        ...
        image_loader = SimpleITKIO()
        # Assuming single modality for BCBM, hence only _0000
        full_image_path = [os.path.join(image_dir, f"{patient_id}_0000.nii.gz")]
        ...
        # --- Adversarial Prediction (on target model) ---
        with torch.no_grad():
        ...
        # --- Save Visualizations (with Gradient Heatmap) ---
        fig, axs = plt.subplots(1, 4, figsize=(20, 6))
        
        # Panel 1: Clean Image (using the first and only modality)
        original_slice_np = img_tensor_normalized[0, 0, slice_idx, :, :].cpu().numpy()
        axs[0].imshow(original_slice_np, cmap='gray')
        axs[0].set_title(f"Clean Image\nMetastasis Dice: {result_data['dice_orig_Metastasis']:.3f}")
        axs[0].axis('off')

        # Panel 2: Gradient Map Heatmap
        if data_grad is not None:
            grad_slice_np = data_grad[0, 0, slice_idx, :, :].cpu().abs().numpy()
        ...
        perturbation_slice_np = (adv_image_tensor - img_tensor_normalized)[0, 0, slice_idx, :, :].cpu().numpy()
        ...
        adv_slice_np = adv_image_tensor[0, 0, slice_idx, :, :].cpu().numpy()
...
if __name__ == "__main__":
    ...
    # NOTE: Using 'bcbm' patient ID format. Adjust range as needed for your dataset.
    patient_cases_for_attack = [f"bcbm_{i:03d}" for i in range(30, 60)] # 30 patients for attack demo
    slices_to_attack = [75] # A central slice. May need adjustment depending on image dimensions.
    
    # --- Phase 1: (Optional) Adversarial Fine-Tuning ---
    if PERFORM_ADVERSARIAL_TRAINING:
        # Use a different set of patients for tuning to avoid data leakage
        tuning_patient_cases = [f"bcbm_{i:03d}" for i in range(1, 11)] # Use 10 patients for tuning
    ...
    # --- Phase 2: Define All Attack Configurations ---
    ...
    # --- Phase 4: Save and Analyze Results ---
    ...
    # --- Phase 5: Visualization & Statistical Analysis ---
