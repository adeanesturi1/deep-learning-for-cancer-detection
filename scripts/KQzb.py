...
# === Original prediction ===
# IMPORTANT FIX: Access the first element of the tuple output for deep supervision
output = model(img_tensor_normalized)
pred_full_volume = output[0].argmax(dim=1).squeeze(0) # FIX APPLIED HERE!
...
# IMPORTANT FIX: Access the first element of the tuple output for deep supervision
output_attack_full = model(img_tensor_attack)[0] # FIX APPLIED HERE!
...
with torch.no_grad():
    # IMPORTANT FIX: Access the first element of the tuple output for deep supervision
    adv_output_full = model(adv_image)[0] # FIX APPLIED HERE!
    adv_pred_full_volume = adv_output_full.argmax(dim=1).squeeze(0)
